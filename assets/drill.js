...
import { PRINTERS, printerQuestions, printerRows } from "./drillprint.js";
import { WIFISEC, wifisecQuestions, apPage } from "./drillwifisec.js";
import { BOARDS, channelQuestions, channelTables } from "./drillchan.js";
import { MACHINES, ribbonQuestions, ribbonPanel } from "./drillribbon.js";
...
export const CONNECTORS = [
  ...
  {
    /* ST was on the objective and missing. It is the one round fibre
       connector a student will meet, and the one they cannot identify by the
       rule they learned for the other two ("square body, push it in"). */
    key: "st", name: "ST fibre", family: "fibre", contacts: 1, countLabel: "ferrule",
    media: "glass fibre",
    carries: "light \u2014 one strand, so a duplex link is two of these rather than one clipped pair",
    mates: "an ST port on older patch panels, media converters and building backbone equipment",
    retention: "a bayonet \u2014 push in and twist a quarter turn, like a lens on a camera",
    reach: "hundreds of metres on multimode, which is most of where you will find it",
    rate: "100 Mb/s and 1 Gb/s on the equipment that commonly uses it",
    look: "A round metal barrel with a knurled collar and one 2.5 mm ceramic ferrule standing " +
      "out of the middle, and a small lug on the side that the twist locks against.",
    lookalike: "sc",
    lookalikeWhy: "Both carry the same 2.5 mm ferrule, so the glass end looks identical. " +
      "Everything else differs: one is a square body you push straight in, the other a round " +
      "barrel you twist. If you are trying to push a round fibre plug into a square port you " +
      "have the wrong one, and forcing it damages the end face on both."
  },
  {
    key: "minidp", name: "Mini-DisplayPort", family: "video", contacts: 20,
    countLabel: "pins", media: "copper",
    carries: "video and audio, the same signal the full-size connector carries",
    mates: "a Mini-DisplayPort socket, and physically a Thunderbolt 1 or 2 socket, which is the " +
      "trap \u2014 the plug fits, and what happens next depends on the port rather than the plug",
    retention: "friction only, with a shallow latch on some plugs \u2014 nothing you have to release",
    reach: "two metres passive, further with an active cable",
    rate: "the same as the full-size connector of the same generation",
    look: "A small flat rectangle with two clipped corners at one end, about a third the width " +
      "of the full-size version, with twenty pins in a single row.",
    lookalike: "displayport",
    lookalikeWhy: "The same signal and the same pin count on two very different bodies, which " +
      "is why an adapter for one is useless for the other. The full-size plug has a latch you " +
      "must press to release and this one does not \u2014 pull a latched one out by the cable and " +
      "you take the port with it."
  },
];
...
/* HOW MANY ITEMS A STUDENT CAN ACTUALLY REACH.

   This was a flat 5, and every subject's pool is fourteen to twenty items
   deep. `buildDrillItem` indexes a shuffle of the whole pool, so slots 6
   upward simply did not exist: two-thirds of everything built here was
   unreachable in any given session, and a student who worked all five and
   pressed "new set" got a reshuffle rather than the rest of the subject.

   The count now comes from the pool, so every item is reachable and the
   picker cannot offer a slot that has nothing behind it. Kept as an export
   for the pages that show "item N of M". */
export const DRILL_SLOTS = 5;   /* retained: some call sites want a default */

export function drillSlotsFor(subject) {
  var sub = SUBJECTS[subject];
  if (!sub) return DRILL_SLOTS;
  /* A subject may cap its own count. Mixed draws from every pool at once —
     two hundred and seventy-one slots would not be a session, it would be a
     term — so it declares a length instead of inheriting one. */
  if (sub.slots) return sub.slots;
  return sub.pool ? sub.pool.length : DRILL_SLOTS;
}
...
/* ---------------------------------------------------------------------
   ...
   on the same seed.
   --------------------------------------------------------------------- */
/* Filled in after SUBJECTS exists, because it draws from every subject in
   it. Declared here because the registry holds the reference: populating the
   same array later is what breaks the circle between "mixed needs every
   pool" and "every pool lives in the thing mixed is part of". */
const MIXED_POOL = [];
...
export const SUBJECTS = {
  ...
  netconf: {
    ...
    tag: "On the address space",
    panelTitle: "What the machine reports",
    pool: NETCONF,
    panel: readoutRows,
    questions: function (D) {
  ...
  devices: {
    ...
    tag: "On the bench",
    panelTitle: "What is printed on the case",
    pool: DEVICES,
    panel: plateRows,
    questions: function (D) {
  ...
  soho: {
    ...
    tag: "On the plan",
    panelTitle: "What the customer told you, and what the survey found",
    pool: SITES,
    panel: briefRows,
    questions: function (D) {
  ...
  links: {
    ...
    tag: "On the ground",
    panelTitle: "What you measured, standing at the site",
    pool: LINKS,
    panel: readingRows,
    questions: function (D) {
  ...
  tools: {
    ...
    tag: "In your hand",
    panelTitle: "What the tool is showing",
    pool: TOOLS,
    panel: panelRows,
    questions: function (D) {
  ...
  panels: {
    ...
    tag: "Cut through",
    panelTitle: "What the instruments read",
    pool: PANELS,
    panel: meterRows,
    questions: function (D) {
  ...
  ram: {
    ...
    tag: "On the bench",
    panelTitle: "What a ruler and the label give you",
    pool: MODULES,
    panel: labelRows,
    questions: function (D) {
  ...
  storage: {
    ...
    tag: "Opened up",
    panelTitle: "What the enclosure and the benchmark give you",
    pool: DRIVES,
    panel: benchRows,
    questions: function (D) {
  ...
  psu: {
    ...
    tag: "On the bench",
    panelTitle: "The parts list, and what each line draws",
    pool: JOBS,
    panel: listRows,
    questions: function (D) {
  ...
  /* 3.7 and 3.8. The build had laser and inkjet as full ticket tracks and
     nothing at all for the other three families the objective names —
     thermal, impact and 3D had no coverage anywhere. This is the
     recognition half: what marks the page, what you replace, and the one
     question only an impact machine answers yes to. */
  printers: {
    key: "printers", offset: 7320443,
    tag: "On the bench",
    panelTitle: "What the plate and the call-out say",
    pool: PRINTERS,
    panel: printerRows,
    questions: function (D) {
      return printerQuestions(D, function (list) { return R(D.seedBase + 331).shuffle(list); });
    }
  },
  /* MIXED. Every other subject at once, and the student is not told which.

     Every other subject tells you its objective before you look at the item,
     which is a real advantage the exam does not give and the bench does not
     give either. This deals from all of them and delegates
     everything — questions, model, panel, caption — to whichever subject the
     item actually came from, so the only thing that changes is that nobody
     says which one it is first.

     The pool is the same object references the other subjects hold, tagged
     with the subject they came from. Copies would drift the first time an
     item was edited. */
  /* 2.2 and 2.6, the half neither of them was teaching. The wireless drill
     covers bands and reach; the SOHO drill covers addressing and channels.
     Encryption — a headline topic on both objectives — was graded nowhere.

     Like 2.1 it has no object: a security mode is a choice on a page, so the
     page IS the instrument, and like the firewall table it shows evidence
     rather than names. */
  wifisec: {
    key: "wifisec", offset: 6127339,
    tag: "On the access point",
    pool: WIFISEC,
    instrument: "page",
    questions: function (D) {
      return wifisecQuestions(D, function (list) { return R(D.seedBase + 173).shuffle(list); });
    },
    rules: function (D) {
      return apPage(D, function (list) { return R(D.seedBase + 191).shuffle(list); });
    }
  },
  /* 3.3, the half the RAM drill did not reach. That one identifies modules;
     the board drill asks which slots once, with identical modules, as one
     decision out of five. Nothing graded what mode a machine ends up in,
     what mismatched modules do, or — the one that matters — what fitting
     them wrongly looks like, which is nothing at all.

     Generated rather than pooled: the board is the item, and the modules,
     the speeds and the wrinkle all come from the seed. */
  channels: {
    key: "channels", offset: 6733921,
    tag: "On the bench, nothing fitted yet",
    pool: BOARDS,
    instrument: "page",
    questions: function (D) {
      return channelQuestions(D, function (list) { return R(D.seedBase + 211).shuffle(list); });
    },
    rules: function (D) { return channelTables(D); }
  },
  /* THE RIBBON PATH. Objective 3.7's other half: the printer-types drill
     names a thermal transfer machine and says the ribbon is the whole
     difference, and then never asks anybody to thread one.

     The machines are built left-handed and right-handed, which is the
     reason this is an exercise rather than a diagram. Learn the path off
     one machine and the mirrored one defeats you; learn that the coated
     side faces the label and both are the same machine. Four of the
     seventeen are direct thermal, where the right answer is that there is
     no ribbon and nothing to thread. */
  ribbon: {
    key: "ribbon", offset: 5518273,
    tag: "On the bench, cover open",
    pool: MACHINES,
    panelTitle: "What the plate on the machine says",
    panel: ribbonPanel,
    questions: function (D) {
      return ribbonQuestions(D, function (list) { return R(D.seedBase + 269).shuffle(list); });
    }
  },
  mixed: {
    key: "mixed", offset: 8410729,
    tag: "Nobody has told you what this is",
    slots: 20,
    pool: MIXED_POOL,
    questions: function (D) { return homeOf(D).questions(D); },
    panel: function (D) { return homeOf(D).panel ? homeOf(D).panel(D) : null; },
    panelTitle: function (D) { return homeOf(D).panelTitle; },
    rules: function (D) {
      var home = homeOf(D);
      return home.rules ? home.rules(D) : null;
    }
  },
};

/* Tag every item with the subject it came from and put it in the mixed pool.
   The same object references, not copies — a copy would stop matching the
   moment somebody edited the original, and the questions filter against the
   original pools by identity. */
Object.keys(SUBJECTS).forEach(function (key) {
  if (key === "mixed") return;
  (SUBJECTS[key].pool || []).forEach(function (item) {
    item._home = key;
    MIXED_POOL.push(item);
  });
});
if (MIXED_POOL.length < 100) {
  throw new Error("drill: the mixed pool is only " + MIXED_POOL.length +
    " items. It is built from every subject, so this means the registry was " +
    "read before the pools were attached.");
}

/* Which subject an item in the mixed pool came from. */
export function homeOf(D) {
  var home = D.item && D.item._home;
  if (!home || !SUBJECTS[home]) {
    throw new Error('drill: a mixed item carries no home subject. Every entry in ' +
      'MIXED_POOL must be tagged, or its questions and model cannot be found.');
  }
...
export const DRILL_OBJECTIVES = {
  ...
  printers: {
    title: "Printer types, consumables and maintenance",
    home: "3.7",
    objectives: ["3.7", "3.8"],
    feeds: ["5.6"],
    pairing: "This is objectives 3.7 and 3.8 together, because the book pairs them and " +
      "because a technician who cannot name the machine cannot service it either. The laser " +
      "and inkjet tracks troubleshoot two families in depth; this is the other three — " +
      "thermal, impact and 3D — and the recognition that has to come before any of it. " +
      "Every discriminator here is something you can see or touch: what marks the page, what " +
      "you would replace, how the paper is held, and whether anything actually strikes the " +
      "sheet. That last one is not trivia. Only a machine that strikes the page can make a " +
      "carbon copy, which is why multipart forms still come off impact printers in 2026, and " +
      "it is the question students most often get wrong.",
    modelKind: "printerbench"
  },
  ribbon: {
    title: "Thermal transfer \u2014 threading the ribbon",
    home: "3.7",
    objectives: ["3.7"],
    feeds: ["3.8", "5.6"],
    pairing: "This is the other half of objective 3.7. The printer-types drill gets you as far " +
      "as naming a thermal transfer machine and knowing the ribbon is what makes it one; this " +
      "asks you to put the ribbon in. It matters because the machines are built both ways " +
      "round \u2014 supply on the left on one, on the right on its twin \u2014 so the path is not " +
      "something you can learn once by its shape. One rule survives the mirror: the coated side " +
      "faces the label, never the head. Everything else on this bench is derived from it, " +
      "including which way the roll comes off. Get that and 3.8 follows, because most of what " +
      "maintenance means on these machines is the head, and most of what kills a head is a " +
      "ribbon that was fitted wrong.",
    modelKind: "printerbench"
  },
  wifisec: {
    title: "Wireless security \u2014 what protects the traffic",
    home: "2.2",
    objectives: ["2.2", "2.6"],
    feeds: ["2.5", "5.5"],
    instrument: "page",
    pairing: "This is the security half of 2.2, spent on 2.6. The other wireless drill asks " +
      "where the signal gets to; this one asks who can read it when it gets there, and it is " +
      "the half that costs money when it is wrong. There is nothing to photograph \u2014 a security " +
      "mode is a choice on a page \u2014 so the instrument is that page, with the names taken off, " +
      "because a technician who can only recognise the word WPA2 cannot tell it from WPA on a " +
      "unit whose menu is in another language. Three questions carry most of it: what does it " +
      "actually encrypt, where does the key come from, and what stops working if you turn it " +
      "on. Two of the seventeen protect nothing at all and are the two a customer will tell " +
      "you they have already done. Get this straight and 2.6 becomes a decision rather than a " +
      "default, and 5.5 gets shorter, because \u201cthe new laptop will not join\u201d is usually a " +
      "mode the client cannot do rather than a fault.",
    modelKind: "none"
  },
  channels: {
    title: "Memory channels \u2014 which slots, and what it runs at",
    home: "3.3",
    objectives: ["3.3"],
    feeds: ["3.5", "5.1"],
    instrument: "page",
    pairing: "This is the other half of 3.3. The RAM drill identifies the module; this one puts " +
      "it in the machine, and it exists because of one fact: FITTING MEMORY IN THE WRONG SLOTS " +
      "DOES NOT FAIL. The machine posts, boots, counts every megabyte, logs nothing and beeps " +
      "nothing \u2014 and runs at half the memory bandwidth for the rest of its life. There is no " +
      "fault, so no method in domain five will ever find it; the only way it gets caught is " +
      "somebody opening the case. Around that sit the three questions a build actually asks: " +
      "which slots, what mode that leaves you in, and what speed it really runs at \u2014 which is " +
      "not the number on the box until somebody enables the profile. Get this solid and 3.5 " +
      "gets easier, and 5.1 gets shorter, because \u201cit is slower than the old one\u201d stops " +
      "being a mystery.",
    modelKind: "none"
  },
  mixed: {
    title: "Mixed \u2014 the objective is not given",
    home: "1.x",
    objectives: [],
    feeds: [],
    pairing: "Every other drill tells you its objective before you look at the item, which " +
      "is an advantage the exam does not give you and a bench does not give you either. Half " +
      "of identifying something is knowing what KIND of thing it is, and being told 'this is " +
      "objective 3.2' has already done that half for you. This deals from every other " +
      "subject at once and says nothing. Work out what you are looking at first, then answer " +
      "the same questions you would have answered anyway — twenty items, drawn across the " +
      "whole of Core 1.",
    modelKind: "mixed"
  },
};
