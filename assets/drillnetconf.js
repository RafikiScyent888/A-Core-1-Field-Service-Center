...
export const NETCONF = [
  ...
  {
    /* The pool covered A, AAAA, MX, TXT and CNAME and stopped, which left
       the two records a technician actually meets while diagnosing DNS \u2014
       who is authoritative, and what a reverse lookup answers \u2014 untaught. */
    key: "nsrecord", name: "NS record", kind: "dns",
    what: "A line naming which servers are authoritative for a zone",
    does: "Tells every other resolver where to go for the real answers about this zone, " +
      "which is how a delegation is made and how it is found",
    breaks: "Nothing in the zone resolves from outside, even though the records inside it " +
      "are perfectly correct \u2014 because nobody can find out who to ask",
    where: "At the top of the zone, and in the parent zone that delegates to it",
    reads: "A lookup returns the names of the servers to ask, not an address for the host " +
      "you asked about",
    lookalike: "cname",
    lookalikeWhy: "Both hand back a name rather than an address, so both look like a " +
      "redirection. One is saying 'that host is really called this'; the other is saying " +
      "'I am not the one to ask, go here'. A zone that resolves internally and not " +
      "externally is almost always this one, and almost never the alias."
  },
  {
    key: "ptrrecord", name: "PTR record", kind: "dns",
    what: "A line that answers the question the other way round: given an address, which " +
      "name belongs to it",
    does: "Lets a log, a mail server or a security tool turn an address it has seen into a " +
      "name a person can read",
    breaks: "Forward lookups keep working perfectly, so nothing looks wrong \u2014 but mail " +
      "starts being refused and logs fill with bare addresses, and neither symptom points " +
      "anywhere near DNS",
    where: "In the reverse lookup zone, which is a separate zone from the forward one and " +
      "is very often the one nobody updated",
    reads: "A lookup on an address returns a name",
    lookalike: "arecord",
    lookalikeWhy: "They are the same fact written in opposite directions, and people assume " +
      "creating one creates the other. It does not: they live in different zones, and a host " +
      "with a perfect forward record and no reverse one is the commonest DNS fault there is."
  },
];
