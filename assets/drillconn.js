...
export const LINKS = [
  ...
  {
    /* The network-type half of 2.7 had every shape except the one that is not
       a shape at all \u2014 a mesh has no centre, and a student who has only met
       star-shaped networks reads a mesh plan as a broken one.

       Written to the "scope" shape, not the "service" one. The first draft
       copied satellite's fields (arrives / goesTo / shared) onto a network
       item that wanted reach / spans / owns, and the item rendered as an
       empty bench with no questions on it. */
    key: "wmn", name: "Wireless mesh network", kind: "scope",
    reach: "One site, extended hop by hop rather than by running more cable",
    spans: "Access points that reach the wired network through each other rather than each " +
      "having its own cable home",
    owns: "You do \u2014 every node in it, though not the air they share",
    limit: "Every hop costs throughput, because each node repeats traffic on the same air it " +
      "received it on. A node four hops from the exit has a fraction of the capacity of one " +
      "sitting beside it",
    use: "Covering a space where cabling every access point is impractical \u2014 a warehouse, a " +
      "listed building, a site spread over ground you cannot dig up",
    look: "Several access points with no cable to most of them, and no single point that " +
      "everything hangs off.",
    lookalike: "wlan",
    lookalikeWhy: "Both are wireless and both serve the same clients, so from a laptop they are " +
      "indistinguishable. The difference is behind them: in one, every access point has a cable " +
      "home; in the other, most reach the wire through each other. That is why one degrades " +
      "with distance from the exit and the other does not."
  },
  {
    /* The one I missed. 2.7 draws the distinction and it is the reason a
       service sold as "fibre" still needs a DSL modem in the house — a
       technician who does not know it argues with the customer about what
       they were sold instead of terminating the copper properly. */
    key: "fttc", name: "Fibre to the cabinet", kind: "service",
    arrives: "A telephone pair into the building, exactly as a plain DSL service does",
    goesTo: "A cabinet in the street, where the copper ends and the provider's fibre begins",
    shared: "provider",
    limit: "The copper between the building and the cabinet, and nothing else. Two premises " +
      "on the same product get different speeds because one is two hundred metres from the " +
      "cabinet and the other is nine hundred",
    use: "Where the provider has taken fibre as far as the street but not into the building, " +
      "which is most of the places that call their service fibre",
    look: "A telephone socket on the wall and a cabinet somewhere up the road. Nothing in the " +
      "building looks any different from plain DSL, because nothing in the building IS different.",
    lookalike: "dsl",
    lookalikeWhy: "Inside the premises these are the same installation \u2014 the same pair, the " +
      "same socket, the same modem. The difference is entirely outside, in how far the copper " +
      "runs before it becomes glass, and it is why the speed varies street by street on an " +
      "identical product. A customer told they have fibre and shown a telephone socket is not " +
      "being lied to, and explaining that is part of the job."
  },
];

/* Each kind of item carries a different set of fields, and nothing checked
   that an entry actually had the ones its kind needs. A wireless mesh entry
   written to the wrong shape parsed cleanly, imported cleanly, and rendered
   an empty bench with no questions on it — the failure surfaced only in a
   browser walk. The shape is now asserted when the module loads, naming the
   item and the field, so the next one that is written wrong says so at once. */
const LINK_SHAPE = {
  service: ["arrives", "goesTo", "shared", "limit", "use", "look", "lookalike", "lookalikeWhy"],
  scope: ["reach", "spans", "owns", "limit", "use", "look", "lookalike", "lookalikeWhy"]
};
LINKS.forEach(function (it) {
  const want = LINK_SHAPE[it.kind];
  if (!want) {
    throw new Error('drillconn: "' + it.key + '" has kind "' + it.kind +
      '", which has no field shape. Add one to LINK_SHAPE or fix the kind.');
  }
  const missing = want.filter(function (f) { return !it[f]; });
  if (missing.length) {
    throw new Error('drillconn: "' + it.key + '" (kind ' + it.kind +
      ') is missing ' + missing.join(", ") + '. It would render an empty bench.');
  }
  if (!LINKS.some(function (o) { return o.key === it.lookalike; })) {
    throw new Error('drillconn: "' + it.key + '" names lookalike "' + it.lookalike +
