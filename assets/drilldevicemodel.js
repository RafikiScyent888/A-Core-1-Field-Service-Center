...
function build(it) {
  ...
  switch (it.key) {
    ...
    case "managed":
    case "poeswitch":
    /* Deliberately the same drawing as a managed switch. There is no outside
       difference worth the name between these two, and the item says so —
       giving the layer 3 switch a distinguishing mark here would teach a tell
       that does not exist on a real bench. */
    case "l3switch": {
    ...
    case "dslmodem": {
      ...
      dev = s.dev;
      ports = portField(1, s, P, 0.5);
      /* The one thing that separates this from the cable modem: a socket too
         small for a network lead, on the same flank the coaxial barrel sits
         on over there, so the two are compared in the same glance. */
      var dx = s.w / 2 + 0.08;
      ports.push({ shape: "box", size: [0.16, 0.3, 0.34], pos: [dx, s.y, 0.25], r: 0.03,
        shade: P.deep });
      ports.push({ shape: "box", size: [0.1, 0.16, 0.2], pos: [dx + 0.06, s.y + 0.04, 0.25],
        r: 0.02, shade: P.sock });
      ports.push(lamp(0.0, s.y + 0.5, s.fz, P.lamp, { count: 3, step: [0.34, 0, 0] }));
      return { dev: dev, ports: ports, s: s,
        dataAt: [0.5, s.y + 0.02, s.fz], powerAt: [s.w / 2 - 0.3, s.y, -s.d / 2 - 0.05] };
    }
    case "sdncontroller": {
      /* A rack box with almost nothing on the front, which is the point: no
         traffic passes through it. Two management ports, a console port and
         two power inlets, and a wide blank face where a switch would have
         twenty-four holes. */
      s = rackShell(6.6, 2.2, 0.92);
      dev = s.dev;
      ports = [];
      [-0.5, 0.0].forEach(function (x) {
        ports.push(apart(x, s.y + 0.02, s.fz, 0.36, 0.32, P.sock));
      });
      ports.push(divider(0.36, s.y + 0.02, s.fz, 0.66));
      ports.push({ shape: "box", size: [0.42, 0.2, 0.16], pos: [-2.9, s.y - 0.2, s.fz], r: 0.02,
        shade: P.deep, consolePort: 1 });
      ports.push(lamp(-2.9, s.y + 0.22, s.fz, P.lamp, { count: 2, step: [0.3, 0, 0] }));
      ports.push(lamp(1.4, s.y + 0.02, s.fz, P.lamp, { count: 4, step: [0.34, 0, 0] }));
      return { dev: dev, ports: ports, s: s,
        dataAt: [-0.5, s.y + 0.02, s.fz], powerAt: [-2.4, s.y, -s.d / 2 - 0.05] };
    }
    case "ont": {
  ...
  /* A key with no case here used to `return null`, which drew an empty bench
     and left the questions asking about a device that was not on screen. It
     is the fourth table in this build keyed by item that did not grow when
     the items did, so it now says which key is missing instead of failing
     quietly. Anyone adding a device meets this immediately rather than
     shipping an invisible one. */
  throw new Error("drilldevicemodel: no model for device key \"" + it.key +
    "\". Add a case for it, or the bench renders empty.");
}
