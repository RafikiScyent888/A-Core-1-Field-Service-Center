...
export const CABLE_PARTS = [
  ...
  },
  {
    ...
    fails: "Rarely, and when it does it is because somebody turned it off rather than because anything broke.",
    seen: "Dark. A known-good cord and a known-good machine make no difference to it, and the port beside it works with the same cord."
  }
];
...
export const CABLING_FAULTS = [
  ...
  {
    /* The switch port existed on this bench for fifteen tickets as a part
       nobody could ever be right about. It is still almost never BROKEN —
       what this ticket teaches is that "almost never at fault" and "never
       the answer" are different claims, and the difference is a port that
       somebody turned off. */
    key: "portdisabled", part: "a switch port that has been shut down", target: "switchport",
    objective: "2.3 + 5.5", kind: "config",
    root: "The port is administratively disabled. It was shut during a security tidy when the desk was empty, and the desk is not empty any more.",
    observable: "no link light at all on a run that certifies perfectly, and the same cord and the same machine come straight up in the port beside it",
    symptoms: ["This desk has never worked", "The cable's been tested, it's fine",
      "It works if I move to the next desk along"],
    fixes: "Have the port re-enabled, and have whoever shut it record why so the next tidy does not shut it again. Nothing about the cabling needs touching.",
    wrongReflex: "the cable",
    wrongWhy: "The run certifies to its category end to end and the same cord lights the next port along. When the cable passes every test there is and the link still does not come up, what is left is whether the thing at the end of it has been told to answer.",
    evidence: "A link that certifies clean end to end, with the identical cord and machine coming straight up one port over",
    tool: "loopback"
  },
  {
    key: "plugstrain", part: "a plug with no strain relief on the jacket", target: "plugnear",
    objective: "3.2 + 5.5", kind: "termination",
    root: "The crimp caught the conductors and never closed over the jacket. Every pull on the cord is taken by eight thin wires at the contacts instead of by the sheath.",
    observable: "the link drops when the cord is moved and comes back when it is left alone, and a static tester finds nothing wrong",
    symptoms: ["It drops out when I move my leg under the desk",
      "It's fine if nobody touches it", "The tester said it was OK"],
    fixes: "Cut the plug off and re-terminate with the jacket pushed far enough into the plug that the crimp closes over it. The sheath takes the strain or the conductors do.",
    wrongReflex: "switch port",
    wrongWhy: "A port does not come and go with somebody's knee. An intermittent that tracks movement of the cord is mechanical and it is at whichever end is being moved — which is the end you can reach.",
    evidence: "A link that passes a static continuity test and drops repeatably when the cord is flexed at the plug",
    tool: "inspect"
  },
  {
    key: "solidplug", part: "patch cords made from solid-core cable", target: "patchcord",
    objective: "3.1 + 5.5", kind: "termination",
    root: "Somebody made patch cords out of the solid-core box cable meant for the walls. Solid conductors and plugs designed for stranded do not bite the same way, and the cord will not survive being moved.",
    observable: "cords that work on the bench and fail once they are in a rack that gets touched, all of them made at the same time",
    symptoms: ["We made a batch up to save waiting", "They tested fine when we made them",
      "It's three different ones now, not just one"],
    fixes: "Replace them with factory stranded cords of the right category. Solid core belongs in the wall where it never moves; stranded belongs in the cord that does.",
    wrongReflex: "the patch panel",
    wrongWhy: "The panel is punched correctly and the horizontal runs certify. Several cords failing that were all made at once from one box is a batch, not a coincidence, and what they have in common is the cable they were made from.",
    evidence: "Solid conductors in a moving cord, in plugs meant for stranded, on several cords that were all made up together",
    tool: "inspect"
  },
  {
    key: "staplecrush", part: "a run stapled through", target: "horizontal",
    objective: "3.1 + 5.5", kind: "installation",
    root: "The run was stapled to a joist and one staple has been driven home hard enough to crush the jacket and deform the pairs under it.",
    observable: "a link that certifies short of its category and fails at the same distance along the run every time",
    symptoms: ["It only does a gigabit and it's supposed to do ten",
      "It's been like it since the run went in", "The others off the same panel are fine"],
    fixes: "Pull a new run on the same route, clipped rather than stapled, and leave the old one in place and labelled dead. A crushed pair does not recover.",
    wrongReflex: "the outlet",
    wrongWhy: "Re-punching the jack changes nothing about a fault that a reflectometer puts twenty metres into the ceiling. The distance is the diagnosis — it is not at either end, so re-terminating either end is work with a known outcome.",
    evidence: "A reflectometer returning the same distance to the fault on every sweep, well inside the run and nowhere near either termination",
    tool: "tdr"
  },
  {
    /* The link is perfect. That is the point: this one is a fire-safety
       finding rather than a fault, in the same shape as the power track's
       outlets — the thing works and it still has to be reported. */
    key: "plenumjacket", part: "the wrong jacket rating above the ceiling", target: "horizontal",
    objective: "3.1 + 5.5 — escalates", kind: "compliance",
    root: "The run above the suspended ceiling is ordinary PVC-jacketed cable in a space used to return air. It carries data perfectly and it is not rated to be there.",
    observable: "a link that certifies to its full category with nothing wrong with it at all, run in cable whose jacket is marked CM rather than CMP",
    symptoms: ["Nothing's wrong with it, it works fine",
      "We ran it ourselves over a weekend", "There's a box of it left in the store"],
    fixes: "Report it in writing and get plenum-rated cable pulled on the same route. Do not leave it because it works — in a fire, that jacket is what fills the return air path with smoke, and the ceiling it is in is the air path.",
    wrongReflex: "nothing",
    wrongWhy: "\"It works\" is the whole trap. This link will pass every test you own for years, and the day it matters is a day nobody is testing cable. A finding that will never show up on an instrument still gets written down and handed on.",
    evidence: "CM-marked jacket in a plenum-rated ceiling space, on a link that certifies to full category",
    tool: "inspect"
  }
];
...
export function buildCable(r, fault) {
  ...
  var c = {
    ...
    nextDb: r.int(2, 9),
    /* Where along the run the reflectometer puts it, and what is stamped on
       the jacket. Both are read off the ticket rather than fixed, so the
       distance a student is asked to walk to is this ticket's distance. */
    faultMetres: fault.key === "staplecrush" ? r.int(14, 31) : null,
    jacket: fault.key === "plenumjacket" ? "CM (general purpose)" : "CMP (plenum)",
    cordCore: fault.key === "solidplug" ? "Solid" : "Stranded",
