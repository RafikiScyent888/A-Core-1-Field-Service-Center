...
export const LAPTOP_PARTS = [
  ...
  },
  {
    ...
    /* The pack carries both of its failure modes, and they look nothing
       alike. Worn cells are the common one and they leave a pack that looks
       brand new, which is the whole reason the capacity figures matter. */
    seenBy: {
      lapbatthealth: "No swelling anywhere. It sits flat, the case is square, the cover closes flush, and there " +
        "is nothing on this pack to see — what has changed is how much it holds."
    }
  },
  {
  ...
  {
    key: "heatpipe", label: "Heat pipe and cold plate", depth: 3,
    role: "A flattened copper tube that carries heat from the processor die to the fin stack at the fan. It is the only thing moving heat across the machine, and it has no moving parts to fail.",
    fails: "Not by itself. It fails at its ENDS — a cold plate bolted down unevenly or dry of paste, so the heat never gets into the pipe in the first place.",
    seen: "Straight, unmarked, no dents in the flat, and both ends firm. The pipe is doing what a pipe does.",
    seenBy: {
      lappaste: "The pipe itself is perfect. The compound between its cold plate and the die has gone grey and " +
        "crumbly, with a dry ring where it has pushed out from the middle over the years.",
      lappipeloose: "One corner of the cold plate stands proud of the die and you can get a fingernail under it. " +
        "Two of the four sprung screws are barely finger tight, and the paste print underneath is heavy on one " +
        "side and untouched on the other."
    }
  },
  {
  ...
  {
    key: "speaker", label: "Speaker pair and loom", depth: 3,
    role: "Two small drivers in the base, one each side, on a single loom to the board. Everything above them — the codec, the driver, the balance setting — is shared between the two.",
    fails: "One driver goes and the other does not, which is why the complaint is always that the sound has moved to one side.",
    seen: "One driver's cone is torn at the edge. The other is intact, and the loom and its connector are sound.",
    seenBy: {
      lapspeakerloom: "Both drivers are intact. The loom's connector is lying beside its socket on the board rather " +
        "than in it, and there is a crush mark across the loom where the top case screws down onto it."
    }
  },
  {
...
export const LAPTOP_FAULTS = [
  ...
  {
    /* Targeted the keyboard until the speakers existed as a part to point
       at, which graded "name the faulty component" as the keyboard
       assembly on a ticket about a blown speaker. */
    key: "lapspeaker", part: "speaker assembly", target: "speaker", depth: 3,
  ...
  {
    /* Targeted the fan while its own root text said the fan was clean and
       turning. The compound sits between the die and the heat pipe's cold
       plate, and until the pipe existed as a part there was nowhere else
       to put it. */
    key: "lappaste", part: "dried thermal compound", target: "heatpipe",
    objective: "1.1 + 5.1", kind: "component", depth: 3,
  ...
  {
    key: "lappipeloose", part: "heat pipe cold plate", target: "heatpipe",
    objective: "1.1 + 5.1", kind: "component", depth: 3,
    root: "The cold plate is not sitting flat on the die. The sprung screws were tightened one at a time instead of diagonally in stages at the last repair, so the plate pulled down at one corner and stands off at the other.",
    observable: "it throttles under load exactly as a dirty machine would, on a machine that was opened three weeks ago and has a clean fan and clear fins",
    symptoms: ["It's been slow since it came back from repair",
      "The fan's clean, we watched them do it", "It was fine before"],
    fixes: "Clean both faces, apply fresh compound, and torque the four sprung screws down a turn at a time in the diagonal sequence the manual gives. The order is the repair.",
    wrongReflex: "cooling fan",
    wrongWhy: "The fan is spotless and at full speed, and the fins are clear — this machine was serviced three weeks ago. A cooler that is clean and moving air and still cannot hold temperature is not failing to move heat away; it is not being given any, and the plate is where heat gets in.",
    evidence: "A paste print heavy on one side of the die and untouched on the other, with two of the four sprung screws barely finger tight"
  },
  {
    key: "lapspeakerloom", part: "speaker loom and connector", target: "speaker",
    objective: "1.1 + 5.4", kind: "component", depth: 3,
    root: "The speaker loom was never plugged back in after the last top-case job, and it has been pinched under a screw boss since. Neither driver has had a signal in weeks.",
    observable: "no sound at all from the machine's own speakers, with headphones perfect and every setting correct",
    symptoms: ["There's no sound at all now", "Headphones work fine",
      "It's not muted, I've checked it about ten times"],
    fixes: "Reseat the speaker connector and dress the loom clear of the screw boss before the case goes back down. Check the crush mark has not cut through — if it has, the loom is the part.",
    wrongReflex: "one of the speakers",
    wrongWhy: "One failed driver moves the sound to one side. BOTH sides silent, on one loom with one connector, points at the thing they share rather than at two parts failing the same week.",
    evidence: "Both channels correct through headphones, neither speaker producing anything, and one connector serving both of them"
  },
  {
    key: "lapbatthealth", part: "battery pack", target: "battery",
    objective: "1.1 + 5.4", kind: "wear", depth: 2,
    root: "The pack has aged. Its full-charge capacity has fallen to a third of what it was designed for, so the percentage is honest about a battery that is now very small.",
    observable: "it reports a full charge and then drops from sixty per cent to nothing in a minute or two, with no swelling anywhere",
    symptoms: ["It says 80% and then just dies", "It lasted all day when it was new",
      "It's fine as long as it's plugged in"],
    fixes: "Fit the correct replacement pack and send the old one out as hazardous waste. There is nothing to repair and nothing to recalibrate — the cells are worn out, which is what cells do.",
    wrongReflex: "the charger",
    wrongWhy: "The charger is delivering its rated output and the machine runs indefinitely on mains. What has gone is how much the pack can hold, and no charger fixes a container that has got smaller.",
    evidence: "A design capacity and a full-charge capacity that differ by a factor of three, on a pack with no swelling and a healthy cycle count for its age"
  },
  {
    key: "lapadaptive", part: "adaptive brightness", target: "none",
    objective: "1.1 + 5.4", kind: "config", depth: 1,
    root: "Adaptive brightness is on and the ambient light sensor is doing exactly what it was told to. The screen dims and brightens on its own as the light in the room changes, and as the machine is moved.",
    observable: "the brightness changes by itself, more in some rooms than others, and always settles again after a moment",
    symptoms: ["The screen keeps dimming on its own", "It's worse by the window",
      "I keep turning it up and it goes back down"],
    fixes: "Turn adaptive brightness off with the user watching, and show them where the setting is so they can put it back if they want it. Nothing comes apart on this ticket.",
    wrongReflex: "the display assembly",
    wrongWhy: "A failing panel or backlight does not recover. This settles at a new level every time and tracks the light in the room, which is a sensor and a setting behaving correctly — and quoting a display assembly for it is an expensive way to be wrong.",
    evidence: "Brightness tracking the room's light level and settling, with adaptive brightness enabled in settings"
  },
  {
    key: "lappowerplan", part: "power plan on battery", target: "none",
    objective: "1.1 + 5.1", kind: "config", depth: 1,
    root: "The power plan drops the processor to its lowest state whenever the machine is on battery. Nothing is overheating and nothing is failing — it has been configured to be slow.",
    observable: "it is slow only on battery and instantly normal the moment the charger goes in, with every temperature inside its limit",
    symptoms: ["It's slow unless it's plugged in", "Plug it in and it's fine straight away",
      "It doesn't get hot, it's just slow"],
    fixes: "Set the battery power plan to something the user can work on, and explain the trade against runtime so they choose it rather than have it chosen for them.",
    wrongReflex: "thermal throttling",
    wrongWhy: "Thermal throttling follows temperature and takes minutes to arrive. This follows the charger and arrives instantly, at temperatures in the fifties — which is a policy rather than a physical limit, and no amount of cleaning will change it.",
    evidence: "Clock speed pinned to its floor on battery and full boost on mains, at identical load and identical temperature"
  }
];
...
export const LAPTOP_ACTIONS = [
  ...
  { key: "platetorque", label: "Clean both faces, apply fresh compound, and torque the plate down in the manual's diagonal sequence" },
  { key: "speakerconn", label: "Reseat the speaker connector and dress the loom clear of the screw boss" },
  { key: "battfit", label: "Fit the correct replacement pack for this model" },
  { key: "setting", label: "Change the setting with the user watching, and show them where it is" },
...
const PROCEDURES = {
  ...
  lapboard: ["quote", "backup", "poweroff", "esd", "covers", "battdisc", "topcase",
    "reassemble", "battconn", "boot", "test"],
  /* The newest five. The two configuration ones open nothing at all, which
     is the answer as much as the setting is: a machine that is slow by
     policy and a screen that is dim by design are both jobs where the right
     move is to touch a setting in front of the user and go. */
  lappipeloose: ["quote", "backup", "poweroff", "esd", "covers", "battdisc", "topcase",
    "platetorque", "battconn", "reassemble", "boot", "test"],
  lapspeakerloom: ["quote", "backup", "poweroff", "esd", "covers", "battdisc", "topcase",
    "speakerconn", "battconn", "reassemble", "boot", "test"],
  lapbatthealth: ["quote", "backup", "poweroff", "esd", "covers", "battdisc", "battout",
    "battfit", "hazbin", "reassemble", "boot", "test"],
  lapadaptive: ["quote", "setting", "test"],
  lappowerplan: ["quote", "setting", "test"]
};
...
export function laptopProcedureWhy(fault, key) {
  ...
  var deeper = {
    ...
    platetorque: "The cold plate on this machine is seated flat and its compound is good. Breaking a working " +
      "thermal joint to remake it is how a machine comes back worse than it went in.",
    speakerconn: "The speaker loom is seated and undamaged on this ticket.",
    battfit: "The pack on this machine holds what it was built to hold. A new one is a part the customer buys " +
      "for nothing.",
    setting: "Nothing on this ticket is a setting. Changing one in front of the user makes it look as though " +
      "something was done, and leaves the fault exactly where it was."
  };
  return deeper[key] ||
...
const TESTS = [
  ...
  { key: "pasteprint", label: "Take the cold plate off and read the paste print on the die", mins: 18, isolates: "lappipeloose",
    hit: "The print is heavy on one side of the die and the other side has never been touched. Two of the four sprung screws are barely finger tight. The plate has been sitting on one corner since the last repair.",
    miss: "An even print across the whole die, and all four screws to the same tension." },
  { key: "bothsides", label: "Play a hard-panned test tone and listen to each side in turn", mins: 4, isolates: "lapspeakerloom",
    hit: "Neither side produces anything at all, and the same tone through headphones puts each channel exactly where it should be. Two drivers do not fail in the same week; one connector serves both.",
    miss: "One side plays and the other does not, which is one driver rather than what they share." },
  { key: "capacity", label: "Compare the pack's design capacity with its full-charge capacity", mins: 5, isolates: "lapbatthealth",
    hit: "Full-charge capacity is about a third of design capacity, with a cycle count that is unremarkable for the age and no swelling anywhere. The percentage is honest about a battery that has got small.",
    miss: "Full-charge capacity is close to design capacity. The pack holds what it was built to hold." },
  { key: "lightroom", label: "Carry it between a bright room and a dark one and watch the screen", mins: 4, isolates: "lapadaptive",
    hit: "The brightness follows the room every time and settles at a new level, and it does the same thing on a fresh user profile. That is a sensor and a setting doing their job.",
    miss: "Brightness holds wherever you carry it." },
  { key: "unplugged", label: "Run the same benchmark on mains and on battery, watching clock and temperature", mins: 12, isolates: "lappowerplan",
    hit: "Full boost clock on mains and the floor clock on battery, at the same load and the same temperature, switching the instant the charger moves. Nothing here is hot.",
    miss: "Identical clocks on mains and on battery." },
