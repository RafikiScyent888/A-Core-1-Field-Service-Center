...
export const CONNECTOR_BUILD = {
  ...
  /* The round fibre connector. A knurled barrel with a bayonet slot, and a
     single ferrule standing proud of it — one strand, not a duplex pair,
     which is half of what separates it from the two square ones. */
  st: [
    { shape: "cyl", size: [1.05, 1.25], pos: [0, 0, 0], rot: [P2, 0, 0], seg: 18, shade: 1.2 },
    { shape: "torus", size: [1.08, 0.07], pos: [0, 0, 0.32], rot: [0, 0, 0],
      repeat: { count: 5, step: [0, 0, -0.14] }, seg: 18, shade: 1.05 },
    { shape: "cyl", size: [0.42, 1.5], pos: [0, 0, 0.85], rot: [P2, 0, 0], seg: 16, shade: 1.5, contact: 1 },
    /* the bayonet lug — the tell. It twists on rather than clicking in. */
    { shape: "rbox", size: [0.22, 0.3, 0.34], pos: [0.98, 0, -0.15], r: 0.04, shade: 0.9 },
    { shape: "rbox", size: [1.5, 0.6, 0.5], pos: [0, 0, -1.0], r: 0.07, shade: 0.6 }
  ].concat(tail(1.9, 0.4, -1.35)),

  /* Same signal as the full-size one, on a body barely a third the width —
     which is exactly why it gets mistaken for something it is not. */
  minidp: [
    { shape: "rbox", size: [1.5, 0.62, 1.5], pos: [0, 0, 0], r: 0.06, shade: 1.2 },
    { shape: "plate", size: [1.3, 0.05, 1.2], pos: [0, 0.3, 0.05], shade: 0.95 }
  ].concat(contacts(10, -0.54, 0.12, 0, 0.55, 0.07, 0.3, 0.5, 1.5))
   .concat([{ shape: "rbox", size: [1.7, 0.66, 0.5], pos: [0, 0, -1.0], r: 0.07, shade: 0.62 }])
   .concat(tail(1.9, 0.4, -1.35)),
...
const BODY = {
  ...
  lc: "#4a8ec2", sc: "#4a8ec2", st: "#4a8ec2",
  minidp: "#8f99a3", ftype: "#a9b0b7", bnc: "#a9b0b7"
};
...
export function connectorModel(D) {
  var it = D.item;
  /* Two tables keyed by the item, and neither had a floor: a connector added
     without a build drew nothing at all and the questions asked about an
     empty bench. That is the fifth table in this build to do it, so this one
     says which key is missing rather than failing quietly. */
  if (!CONNECTOR_BUILD[it.key]) {
    throw new Error('drillmodels: no geometry for connector "' + it.key +
      '". Add it to CONNECTOR_BUILD, or the bench renders empty.');
  }
  if (!BODY[it.key]) {
    throw new Error('drillmodels: no body colour for connector "' + it.key + '". Add it to BODY.');
  }
  return {
    kind: "drill",
    title: "On the bench",
